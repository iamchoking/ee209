...
int add_cmd(DynArray_T tokens, DynArray_T cmds, int startI, int endI){
	...
	for(int i = startI;i<=endI;i++){
		...
		if(cmd->argv[i-startI]==NULL){err_alloc(PN);return FAIL;}
	...
	if(!DynArray_add(cmds,cmd)){return FAIL;}
	return SUCCESS;
}
